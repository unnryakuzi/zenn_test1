...
- メッシュ分割
縦 $0.1 \mathrm{\;m}$、横 $0.1 \mathrm{\;m}$の要素に分割します(変数名は`size`)。今回の直方体は縦 $1 \mathrm{\;m}$、横 $2 \mathrm{\;m}$なので、$y$ 方向に $10$ 分割、$x$方向に $20$ 分割され、合計で $200$ 個の要素に分かれます(図 $4$)。それぞれの要素の節点で解(変位)を求め、すべての節点の変位を足し合わせることで、全体の変形の様子が分かります。
- 要素番号の割り振り

...
例えば図 $2$ のような形状および荷重条件に対して物体の変位を求めることができます。
...
- 解析モデルの寸法
解析モデルは縦 $1 \mathrm{\;m}$、横 $2 \mathrm{\;m}$、奥行 $1 \mathrm{\;m}$の直方体です。計算コストを考え二次元として解析しています。(変数名は`rect`)
- 拘束条件
図 $3$ の青線で示すように、左の端面がどの方向にも動かないように固定します。つまり直方体が壁に刺さっているような状態です。
- 荷重条件
図 $3$ の赤丸で示すように拘束をかけた反対側の端面の上端と下端を $10\times10^6 \mathrm{N}$(ニュートン)で引っ張ります。
...
- メッシュ分割
今回の直方体は縦 $1 \mathrm{\;m}$、横 $2 \mathrm{\;m}$です。その直方体を縦 $0.1 \mathrm{\;m}$、横 $0.1 \mathrm{\;m}$の要素に分割します(変数名は`size`)。したがって、$y$ 方向に $10$ 分割、$x$方向に $20$ 分割され、合計で $200$ 個の要素に分かれます(図 $4$)。それぞれの要素の節点で解(変位)を求め、すべての節点の変位を足し合わせることで、全体の変形の様子が分かります。
- 要素番号の割り振り
図 $3$ の赤数字で示す通り、左上から右下へ要素番号を割り振っています。
- 材料物性値
今回は合金鋼(ヤング率: $2.1\times10^ {11} \mathrm{N/m^{2}}$、ポアソン比: $0.28$)の物性値を入れています。
![](https://storage.googleapis.com/zenn-user-upload/fc6aa5e90d83-20211219.jpg)
*Fig. $4$ 要素番号の割り振り方*
...
### 2.2 変数の定義(いじらない部分)
```python
# 四角形要素(三角形要素に比べて要素内のひづみが分布しているので精度が良い)
...
今回の問題では、既知数である全体剛性マトリックス $[K]$ および節点力 $\{F\}$ の情報を用いて、未知数である変位 $\{U\}$ を解いていきます。
全体剛性マトリックス $[K]$は各要素の剛性マトリックス$[K_{mG}]$を足し合わせることで導出できます。 $[K_{mG}]$は、$m$ 番目の要素の剛性行列 $[K_{m}]$ の行列サイズを、全体剛性マトリックスの行列サイズ(Global)へと拡大したものです。
以下のプログラムは `ElementClass` を定義しています。 `ElementClass` は各要素の剛性マトリックス $[K_m]$ および $[K_m]$の行列サイズを拡大した $[K_{m}]$を作成しています。
...
\lbrack K\rbrack\left\{ U \right\} = \left\{ F \right\} \\
\end{matrix}$$
以下のプログラムでは全体剛性方程式の右辺である節点力$\{F\}$を作成しています。そして先ほど求めた全体剛性マトリックス $[K]$を用いて変位$\{U\}$を導出していきます。
```python
def makeU(K):
...
変形量はあまりにも微小なので結果をそのまま描画すると変形の様子が分かりづらいです。したがって一般的な有限要素法ソフトでは結果を誇張して描画することが多いです。今回はスケールを $410$ 倍としています。(次の章で市販CADソフトSolidWorksを用いて、同条件で解析結果を比較していきます。SolidWorksの解析結果のスケールが $410$ 倍のため本プログラムのスケールも同様としました。)
解析対象に力を加えた箇所(図 $6$の解析対象右下の青丸)の、$x$ 方向の変位は $0.41057 \mathrm{\;mm}$、$y$ 方向の変位は$0.14231 \mathrm{\;mm}$となりました。
...
力を加えている $2$ 箇所の $x$ 方向の変位が大きく、その値は $0.4874 \mathrm{\;mm}$です。(本プログラム...$0.41057 \mathrm{\;mm}$)

- $y$ 方向変位のコンター図
![](https://storage.googleapis.com/zenn-user-upload/4990a0fa99aff443fcdaef73.png)
*Fig. $7$ SolidWorksによる $y$ 方向変位のコンター図*
力を加えている $2$ 箇所の $y$ 方向の変位が大きく、その値の絶対値は $0.2182 \mathrm{\;mm}$です。(本プログラム...$0.14231 \mathrm{\;mm}$)
